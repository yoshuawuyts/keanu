 ...
 * @returns {boolean}
 */
function _isModifier(key) return key == 'shift'
  || key == 'ctrl'
  || key == 'alt'
  || key == 'meta';
...
function _bindSequence(combo, keys, callback, action) {
  ...
  for (var i = 0; i < keys.length; ++i) {
    var val = i < keys.length - 1 ? _increaseSequence : _callbackAndReset;
    _bindSingle(keys[i], val, action, combo, i);
  }
}
